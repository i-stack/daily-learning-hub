...
命令会返回当前最大课程 ID 和下一课程 ID；专题配置课程顺序表时，还会返回下一课主题。《道德经》使用 `DDJ-Dxxx`，《易经》使用 `YJ-Dxxx`，《iOS 底层训练营》使用 `IOS-Dxxx`；新课程必须按原文章次或既定课程表继续递增。《易经》从乾卦开始，严格依六十四卦通行次序发布；iOS 训练营从 RunLoop 开始，严格依 `data/ios/curriculum.json` 发布。写入 JSON 后依次运行 `npm run build` 与 `npm run check`，校验会拒绝错前缀、重复编号、跳号、课程顺序错误和数据索引不同步；全部通过后才可提交和发布。`data/<key>/index.json`、专题列表、课程详情、周索引和知识树均由构建脚本生成，不手工维护。
